...
public interface LocalEventDetailRepository extends JpaRepository<LocalEventDetail, Long> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM LocalEventDetail t WHERE t.contentid = :contentid")
    Optional<LocalEventDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO local_event_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, cpyrht_div_cd, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            "VALUES (:contentid, :contenttypeid, :booktour, :createdtime, :homepage, :modifiedtime, :tel, :telname, :title, :firstimage, :firstimage2, :cpyrht_div_cd, :areacode, :sigungucode, :cat1, :cat2, :cat3, :addr1, :addr2, :zipcode, :mapx, :mapy, :mlevel, :overview) " +
            ...
            nativeQuery = true)
    void upsertLocalEventDetail(@Param("contentid") String contentid,
                                ...
                                @Param("cpyrht_div_cd") String cpyrht_div_cd,
