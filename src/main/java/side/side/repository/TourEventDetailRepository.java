...
public interface TourEventDetailRepository extends JpaRepository<TourEventDetail, Long> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM TourEventDetail t WHERE t.contentid = :contentid")
    Optional<TourEventDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO tour_event_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            ...
            nativeQuery = true)
    void upsertTourEventDetail(@Param("contentid") String contentid,
