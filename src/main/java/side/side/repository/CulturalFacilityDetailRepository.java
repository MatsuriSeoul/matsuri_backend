...
public interface CulturalFacilityDetailRepository extends JpaRepository<CulturalFacilityDetail, Integer> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM CulturalFacilityDetail t WHERE t.contentid = :contentid")
    Optional<CulturalFacilityDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO cultural_facility_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            ...
            nativeQuery = true)
    void upsertCultural(@Param("contentid") String contentid,
