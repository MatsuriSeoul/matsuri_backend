...
public interface TravelCourseDetailRepository extends JpaRepository<TravelCourseDetail, Integer> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM TravelCourseDetail t WHERE t.contentid = :contentid")
    Optional<TravelCourseDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO travel_course_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            ...
            nativeQuery = true)
    void upsertTravelCourseDetail(@Param("contentid") String contentid,
