...
public interface ShoppingEventDetailRepository extends JpaRepository<ShoppingEventDetail, Integer> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM ShoppingEventDetail t WHERE t.contentid = :contentid")
    Optional<ShoppingEventDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO shopping_event_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            ...
            nativeQuery = true)
    void upsertShoppingEventDetail(@Param("contentid") String contentid,
