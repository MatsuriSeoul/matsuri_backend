...
public interface FoodEventDetailRepository extends JpaRepository<FoodEventDetail, String> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT t FROM FoodEventDetail t WHERE t.contentid = :contentid")
    Optional<FoodEventDetail> findByContentidForUpdate(@Param("contentid") String contentid);


    @Modifying
    @Query(value = "INSERT INTO food_event_detail (contentid, contenttypeid, booktour, createdtime, homepage, modifiedtime, tel, telname, title, firstimage, firstimage2, areacode, sigungucode, cat1, cat2, cat3, addr1, addr2, zipcode, mapx, mapy, mlevel, overview) " +
            ...
            nativeQuery = true)
    void upsertFoodEventDetail(@Param("contentid") String contentid,
